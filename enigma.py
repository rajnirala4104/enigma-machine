...
class Rotor:
    def __init__(self, r, left=None, right=None):
        ...
        self.left = left
        self.right = right
        self.rect = pygame.Rect(0, 0, 0, 0)  # Initialize rect with a default value
    ...
    def setPlug(self):
        ...
        for s in sockets:
            for p in plugs:
                ...
                if self.rect.colliderect((sockets[s].x - 20, sockets[s].y - 20, 40, 40)):
                    self.rect.x = sockets[s].x - 20
                    self.rect.y = sockets[s].y - 20
                    rotor[0].r[alpha2index(self.s)] = rotor[0].r[alpha2index(s)]
            ...
            self.rect.y = sockets[self.s].y - 20
            self.picked = False
            picked = False
